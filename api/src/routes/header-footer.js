...
router.post('/update', requireAuth, async (req, res) => {
  try {
    ...
    if (header_layout && !['regular', 'modern', 'mega'].includes(header_layout)) {
    ...
    if (existing.length > 0) {
      await pool.execute(
        `UPDATE header_footer_settings SET
          ...
          header_menu_1 = ?, header_menu_2 = ?, header_mega_html = ?,
          footer_columns = ?, footer_copyright = ?, updated_by = ?
        WHERE site_id = 1`,
        [payload.header_layout, payload.header_logo_url, payload.header_logo_text, payload.header_menu_1, payload.header_menu_2, payload.header_mega_html, payload.footer_columns, payload.footer_copyright, payload.updated_by]
      );
    } else {
      await pool.execute(
        `INSERT INTO header_footer_settings (site_id, header_layout, header_logo_url, header_logo_text, header_menu_1, header_menu_2, header_mega_html, footer_columns, footer_copyright, updated_by)
         VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [payload.header_layout, payload.header_logo_url, payload.header_logo_text, payload.header_menu_1, payload.header_menu_2, payload.header_mega_html, payload.footer_columns, payload.footer_copyright, payload.updated_by]
      );
    }
